...
struct fun_desc {
  char *name;
  void (*fun)(char* suspectedFileName);
};
...
unsigned short swapEndians(unsigned short num)
{
...
virus* readVirus(FILE* input,bool isBigEndian)
{
  ...
  if(isBigEndian)
    toReturn -> SigSize = swapEndians(toReturn -> SigSize);
...
void loadSignatures(char* suspectedFileName)
{
  ...
  if(fgets(fileName,fileNameSize,stdin) != NULL)
  {
    ...
    isBigEndian = header[3] == 'B';
    while((nextVirus = readVirus(virusFile,isBigEndian)) != NULL)
    {
...
void printSignatures(char* suspectedFileName)
{
  if(viruses != NULL)
  {
...
void executeMenu(struct fun_desc* menu,int argc,char **argv)
{
  ...
  while (1) {
    ...
    if(fgets(inputFromUser,bufferSize,stdin)!= NULL)
    {
      ...
      if(argc > 1)
        (menu+selection-1) -> fun(argv[1]);
      else
        (menu+selection-1) -> fun("");
      printf("DONE.\n");
    }
  }
}
...
void detectVirus(char* suspectedFileName)
{
...
void killVirus(char* suspectedFileName)
{
